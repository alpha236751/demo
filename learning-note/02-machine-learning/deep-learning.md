...
## 9.2 词嵌入层
### 9.2.1 什么是词嵌入 Word Embedding
将词映射到向量的技术
- 分词、清洗和标准化
- 构建词表Vocabulary ，将每个词分配一个索引
- 使用词嵌入矩阵将词索引转换为词向量
...
text = "自然语言是由文字构成的，而语言的含义是由单词构成的。即单词是含义的最小单位。因此为了让计算机能够理解自然语言，首先要让它理解单词含义。"
original_text = jieba.lcut(text) #精确分词
stopwords ={"的", "是", "和", "而", "由", "，", "。"} # 停用词
words = [word for word in original_text if word not in stopwords] # 移除停用词

id2word = list(set(words))
word2id = {word: i for i, word in enumerate(id2word)}

embed = nn.Embedding(len(id2word), 5) # 词嵌入层
for word in words:
    word_vec = embed(torch.tensor(word2id[word]))
    print(f"word: {word},\t id: {word2id[word]:<2},\t embed: {word_vec.detach()}")
```

## 9.3 循环网络层
### 9.3.1 RNN层介绍
RNN(Recurrent Neural Network)层，具有环路，通过环路数据在层内循环，实现对序列数据的处理。
...
rnn = nn.RNN(input_size=8, hidden_size=16, num_layers=2) # 输入特征8维，隐状态特征16维 2层RNN
input = torch.randn(3, 3, 8)  # 输入数据 序列长度3， 批量大小3， 输入特征8维
hx = torch.randn(2, 3, 16) # 初始隐状态 2层RNN，批量大小3，隐状态特征16维 

output, hn = rnn(input, hx) 
print(output.shape) # (3, 3, 16) 序列长度3，批量大小3，特征16维 包含所有时间步的隐状态
print(hn.shape) # (2, 3, 16) 2层RNN，批量大小3，特征16维 对应最后一个时间步的隐状态
```
> 对于多层RNN，每一层RNN有各自的初始隐状态，只是在初始统一输入，结束时统一输出。
...
# 加载数据
def poems_preprocess():
    poems_list = []
    char_set = set()
    with open('data/poems.txt', 'r', encoding='utf-8') as f:
        for line in f: # 对每一行进行处理
            line = re.sub(r'[，。？！、：]', '', line).strip() # 格式化 移除标点符号 并去首尾空格 返回字符串
            char_set.update(line) # 字符串转集合 求并集 逐字符
            poems_list.append(line) # 统一作为单一元素处理
    # 构建词表
    id2word = list(char_set) + ['<UNK>']
    word2id = {word: i for i, word in enumerate(id2word)}   
    # 诗句id化
    id_seqs = []
    for poem in poems_list: # 遍历每一句诗 行
        id_seq = [word2id.get(word) for word in poem] # 对每一行的每个字符进行id化
        id_seqs.append(id_seq)
        
    return id_seqs, id2word, word2id  # 返回诗句逐字的id序列 词表 id表 
id_seqs, id2word, word2id = poems_preprocess()
# print(len(id_seqs))
# 构建数据集
class PoetryDataset(Dataset):
    def __init__(self, id_seqs, seq_len):
        self.seq_len = seq_len # 序列长度
        self.data = [] # 数据元组列表<x, y>
        for id_seq in id_seqs: # 遍历每一句诗的序列 行
            for i in range(len(id_seq) - seq_len): # 遍历一句诗的所有可能序列
                self.data.append((id_seq[i:i+seq_len], id_seq[i+1:i+1+seq_len])) # 添加数据元组  
    def __len__(self):
        return len(self.data) # 返回数据集大小
    def __getitem__(self, idx):
        x = torch.tensor(self.data[idx][0]) # 输入序列
        y = torch.tensor(self.data[idx][1]) # 输出序列
        return x, y # 返回输入序列和输出序列
    
dataset = PoetryDataset(id_seqs, seq_len=24)
# print(len(dataset))
class PoetryRNN(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_layers=1):
        super().__init__()
        # 词嵌入层
        self.embed = nn.Embedding(vocab_size, embed_dim) # 词表大小，嵌入维度
        # RNN
        self.rnn = nn.RNN(embed_dim, hidden_dim, num_layers, batch_first=True) # 嵌入维度，隐状态维度，层数
        # 输出层
        self.linear = nn.Linear(hidden_dim, vocab_size) # 隐藏状态维度，词表大小    
...
model = PoetryRNN(vocab_size=len(word2id), embed_dim=256, hidden_dim=512, num_layers=2)
...
def train(model, dataset, lr, epoch_num, batch_size, device):
    ...
    for epoch in range(epoch_num):
        ...
        for batch_idx, (x, y) in enumerate(data_loader):
            ...
            y = y.to(device)
            output, hn = model(x)
            loss_val = loss(output.transpose(1, 2), y)
            loss_val.backward()
            optimizer.step()
            optimizer.zero_grad()
            train_loss += loss_val.item() * x.size(0)
            # print(f"Epoch [{epoch+1}/{epoch_num}], Batch [{batch_idx+1}/{len(data_loader)}], Loss: {loss_val.item():.4f}")
        this_epoch_loss = train_loss / len(dataset)
        print(f"Epoch [{epoch+1}/{epoch_num}], Loss: {this_epoch_loss:.4f}")
...
def generate_poem(model, id2word, word2id, start_token, line_num=4, line_len=7):
    model.eval()
    poems = []
    current_rest_len = line_len
    start_id = word2id.get(start_token, word2id['<UNK>'])
    if start_id != word2id['<UNK>']:
        poems.append(start_token)
        current_rest_len -= 1
    input = torch.tensor([[start_id]]).to(device)
    with torch.no_grad(): # 关闭梯度计算
        for i in range(line_num): 
            for interpunction in ["，", "。\n"]:
                while current_rest_len > 0:
                    output, hn = model(input)
                    prob = output[0,0].softmax(dim=-1)
                    next_id = torch.multinomial(prob, num_samples=1)
                    poems.append(id2word[next_id.item()])
                    current_rest_len -= 1
                    input = next_id.unsqueeze(0)
                poems.append(interpunction)
                current_rest_len = line_len
    return "".join(poems)

for i in range(10):
    print(generate_poem(model, id2word, word2id, start_token="我"))
```
