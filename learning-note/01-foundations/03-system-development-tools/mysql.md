### 一、 MySQL概述
#### 1. 数据库相关概念
- 数据库（Database，DB）：数据库是一种用于存储和管理数据的系统。  
- 数据库管理系统（Database Management System，DBMS）：数据库管理系统是一种用于管理数据库的软件。  
- SQL（Structured Query Language，结构化查询语言）：SQL是一种用于操作数据库的编程语言。  
- 关系型数据库：以二维表的形式存储数据，表与表之间存在关联关系，支持事务
- 主流关系型数据库有MySQL、Oracle等。 
...
- 方式二：系统自带的命令行工具执行指令`mysql [-h 127.0.0.1] [-P 3306] -u root -p`（需要配置环境变量）
...
|CASE WHEN [ expr ] WHEN [ val1 ] THEN [ res1 ] … ELSE [ default ] END|如果expr的值等于val1，返回res1，… 否则返回default|
...
> 父表必须先存在，且被引用的字段必须是主键或唯一键。
> 子表外键字段的数据类型必须和父表主键完全一致。
> 子表插入数据时，外键值必须在父表主键中存在（不能乱填）。
> 删除父表数据 / 父表时，必须先删子表关联数据（否则报错）。
> 一个表可以有多个外键，分别关联不同父表。
