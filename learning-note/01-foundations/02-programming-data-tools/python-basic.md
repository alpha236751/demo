...
### 13. 并发、并行、同步、异步
  ...
  GIL全局解释器锁：在同一个进程内，任意时刻，只有一个线程在执行 Python 字节码
  当线程因为 I/O 操作（如 time.sleep()、requests.get()、open() 读取文件）而被阻塞时，它会主动释放掉 GIL。
...
### 14. 协程
  ...
  - await 只能写在async函数内部
    - 挂起 暂停当前协程执行
    - 等待 安排await后对象(协程对象、Future对象、Task对象)执行 等待其完成
      - 对象有IO操作，事件循环会切换到其他任务执行
      - 对象没有IO操作，不会发生任务切换
    - 恢复 拿到await后对象的结果 恢复之前挂起的协程执行 
  - asyncio.run(async函数) 调用主协程函数 主函数结束时 关闭事件循环 取消尚未完成的task
  - asyncio.gather(async1, async2, ...) 把多个协程对象放进事件循环
  - task1 = asyncio.create_task(work(1, 2)) 返回一个 Task 对象
  - 长时间阻塞同步函数 处理
    1. await asyncio.to_thread(同步函数, 函数参数) **交给线程池**
    2. loop = asyncio.get_running_loop() 拿到事件循环
        await loop.run_in_executor(None, 同步函数, 函数参数) **交给进程池**
        None参数 默认是ThreadPoolExecutor线程池 ProcessPoolExecutor进程池适合CPU密集型
多任务同步执行
```python
import asyncio
import time
...
### 18. 保护成员 和 私有成员
- `_xxx` “保护”类型 
  约定俗成的符号，表示“受保护的”或“内部使用”的属性/方法。Python 解释器不会对它做任何特殊处理。它仅仅是一个提示，告诉其他开发者：“这个函数是类内部或模块内部的实现细节，请勿在外部直接调用。”
- `__xxx` “私有”类型
